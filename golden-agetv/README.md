...
Архив содержит ограниченное количество страниц, поскольку стартовая станица была не начальная golden-agetv.co.uk, а практически "тупиковый" подраздел. Однако именно он представляет наибольшую ценность на этом сайте.
...
У сайта средний индекс архивируемости – 77%. Изучение подробных результатов показало следующее:
1. У сайта низкий показатель доступности, который связан с неудачно организованным кодом, и отсутствием sitemap, который упростил бы архивацию за счет готового представления структуры сайта, и наличием команд disallow в robots.txt. Из-за редиректов между версиями с http и https протоколами этот показатель очень сильно падает.
